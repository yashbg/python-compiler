...
%token NEWLINE ARROW DEF NAME ',' ';' ':' '=' '(' ')' '[' ']' '+' '-' '~' '*' '/' '%'
%token PLUSEQUAL MINEQUAL STAREQUAL SLASHEQUAL PERCENTEQUAL AMPEREQUAL VBAREQUAL CIRCUMFLEXEQUAL LEFTSHIFTEQUAL
%token RIGHTSHIFTEQUAL DOUBLESTAREQUAL DOUBLESLASHEQUAL DOUBLESLASH
%token BREAK CONTINUE RETURN GLOBAL ASSERT IF WHILE FOR ELSE ELIF 
